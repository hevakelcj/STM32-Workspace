...
/**
  ...
  * @retval None
  */
static struct listen_unit s_listen_exit_ucon = {"!@#exit"};
...
void USART1_IRQHandler(void)
{
	...
	if( State & USART_SR_RXNE ){
		...
		if(GetUART1ConnectWith()){
			USART_SendData(GetUART1ConnectWith(),RxData);
			if(Listen(&s_listen_exit_ucon,RxData)){		// ¼àÌý
				SetUART1ConnectWith(USART_1);	
			}
		}else{
			fpost(stdin,RxData);
		}
